<template>
    <div class="object-search-form" @click="closeAllDropMenus">
        <h5 class="object-search-form__title">Цены и бронирование</h5>
        <button class="object-search-form__clear-btn" @click.stop="clearForm">Очистить форму</button>
        <div class="object-search-form__routes object-search-form__input" @click.stop="toggleRoutesMenu">
            <app-airplane-icon />
            <span class="object-search-form__routes-label">Направление: Краснодарский край / {{ routes.selectedRoute ? routes.selectedRoute.name : "" }}</span>
            <div :class="{'object-search-form__routes-drop-menu': true, 'object-search-form__routes-drop-menu--active': routes.isRoutesMenuOpened }">
                <span
                    v-for="city, index in getAllCities"
                    :key="index" 
                    @click.stop="closeRoutesMenu(city)">{{ city.name }}</span>
            </div>
        </div>
        <div class="object-search-form__calendar object-search-form__input">
            <app-calendar-icon />
        ...
        <div class="object-search-form__tax object-search-form__input" @click.stop="toggleTaxMenu(null)">
            <app-tax-icon />
            <span class="object-search-form__tax-label">Тариф / Питание: {{ tax.value ? tax.value : "" }}</span>
            <app-triangle-icon :class="{'object-search-form__input-triangle': true, 'object-search-form__input-triangle--active': tax.isOpened}" />
        ...
        <button
            ...
            @click.stop="$emit('search', {
                ...
                meal_code: tax.code,
                city_id: routes.selectedRoute ? routes.selectedRoute.id : null,
                resort_id: routes.selectedRoute ? routes.selectedRoute.resort_id : null,
            })"
            >
...
import AppAutoCompleteInput from '../inputs/app-auto-complete-input.vue'
import AppAirplaneIcon from '../icons/app-airplane-icon.vue'
import { mapGetters } from "vuex";
    ...
    export default {
       ...
       methods: {
           toggleRoutesMenu(){
               this.routes.isRoutesMenuOpened = !this.routes.isRoutesMenuOpened;
           },
           closeRoutesMenu(data){
               this.routes.isRoutesMenuOpened = false;
               this.routes.selectedRoute = data;
           },
           minusAdult(){
           ...
           clearForm(){
               this.routes.selectedRoute = null;
       ...
       ...mapGetters('cities', ['getAllCities']),
